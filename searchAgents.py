...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        # none of the corners are visited
        visited = (False, False, False, False)
        for i in range(4):
          if(self.startingPosition  == self.corners[i]):
            visited = visited[:i] + (True,) + visited[i+1:]
            ...
            if not hitsWall:
              ...
              for i in range(4):
                if((nextx, nexty) == self.corners[i]):
                  visited = state[1][:i] + (True,) + state[1][i+1:]
                  break;
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        if(state[1][i] == False):
            unvisited.append(problem.corners[i])

    # find distance to from pos to other unvisited
